...
| e2e | The compose-driven e2e runs the browser in the app container's network namespace so the app is reached as `http://localhost` — a secure context; `http://e2e-app` is not, and without Web Crypto the loader (correctly) refuses to verify packs. The loader now says so explicitly. | `docker/compose.yaml`, `playwright.config.ts`, `pack-loader.ts` |
